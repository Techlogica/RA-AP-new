...
class MainActivity : FlutterFragmentActivity() {
    ...
    override fun getDeviceId(): Int {
        val deviceId: Int
        // Add your logic to retrieve the device identifier
        // For example:
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            // Retrieve device ID for Android Q and above
            deviceId = Settings.Secure.getString(contentResolver, Settings.Secure.ANDROID_ID).hashCode()
        } else {
            // Retrieve device ID for Android versions below Q
            deviceId = getSerialNumber()?.hashCode() ?: 0
        }
        return deviceId
    }
    ...
    private fun encryptAndEncode(raw: String, pwd: String): String {
        try {
